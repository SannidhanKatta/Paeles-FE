    ...
    <div className="fixed inset-0 w-full h-[100vh] flex items-center justify-center bg-black/30 overflow-hidden overflow-y-auto z-50">
      <div className="relative w-[95%] h-[80%] md:w-[50%] md:h-[60%] overflow-y-auto dark:text-black bg-white p-10 capitalize">
        ...
        <div className="grid grid-cols-2">
          <div>
            <h2 className="font-bold plus-jakarta text-xl mb-3">Product Details</h2>
            <p><span className="text-md font-semibold mr-1">Id:</span>{userData?._id}</p>
            <p><span className="text-md font-semibold mr-1">Name:</span>{userData?.title}</p>
            <p><span className="text-md font-semibold mr-1">Email:</span>{userData?.email || 'N/A'}</p>
            <p><span className="text-md font-semibold mr-1">Available:</span>{userData?.available}</p>
            <p><span className="text-md font-semibold mr-1">Created At:</span>{new Date(userData?.createdAt).toLocaleString()}</p>
            <p><span className="text-md font-semibold mr-1">Description:</span>{userData?.description}</p>
            <p><span className="text-md font-semibold mr-1">Discount Value:</span>{userData?.discountValue}</p>
            <p><span className="text-md font-semibold mr-1">Main Category:</span>{userData?.mainCategory}</p>
            <p><span className="text-md font-semibold mr-1">Subcategory:</span>{userData?.subCategory || 'N/A'}</p>
            <p><span className="text-md font-semibold mr-1">Price:</span>₹ {userData?.price}</p>
            <p><span className="text-md font-semibold mr-1">Promotional:</span>{userData?.promotional || 'N/A'}</p>
            <p><span className="text-md font-semibold mr-1">Status:</span>{userData?.status}</p>
            <h4 className="mt-4 font-bold">Sizes:</h4>
            {userData?.sizes && userData.sizes.length > 0 ? (
              userData.sizes.map((size, index) => (
                <p key={index}>
                  <span className="text-md font-semibold mr-1"></span>{size.size} - <span className="font-semibold"></span>{size.stock}
                </p>
              ))
            ) : (
